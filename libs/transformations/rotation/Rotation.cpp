...
namespace raytracer::object::primitive {
    Rotation::Rotation(const std::map<std::string, std::any> &params)
        ...
        if (!_wrapped) {
            throw exception::PluginException("Rotation requires a primitive object");
        }

        const double degX = util::ObjectMiddleware::optional<double>(params, "x", 0.0, "Rotation");
        const double degY = util::ObjectMiddleware::optional<double>(params, "y", 0.0, "Rotation");
        const double degZ = util::ObjectMiddleware::optional<double>(params, "z", 0.0, "Rotation");
