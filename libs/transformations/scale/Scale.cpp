...
namespace raytracer::object::primitive {
    Scale::Scale(const std::map<std::string, std::any> &params)
        ...
        if (!_wrapped) {
            throw exception::PluginException("Scale requires a primitive object");
        }

        const double x = util::ObjectMiddleware::validate<double>(params, "x", "scale");
        const double y = util::ObjectMiddleware::validate<double>(params, "y", "scale");
        const double z = util::ObjectMiddleware::validate<double>(params, "z", "scale");
